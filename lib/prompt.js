...
async function addQueries(answer) {
    ...
    switch (answer) {
        case "Who are you adding?": 
            ...
            //role.find((role) => role.title === rows.role_id);

            let manager = await queries.getManager();
            let managerId = manager.map((manager) => `${manager.first_name} ${manager.last_name}`)
           // managers.find((employee) => `${employee.first_name} ${employee.last_name}` === rows.manager_id);
            rows = await queries.addEmployee(roleTitle, managerId);
                break;
