...
async function addEmployee(roles, managers) {
    try {
        ...
        let selectedRole = roles.find((role) => role.title === answer.role)
        let selectedManager = managers.find((manager) => `${manager.first_name} ${manager.last_name}` === answer.manager)

        const SQL_STATEMENT = `INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES (?, ?, ?, ?)`;
        const [rows, fields] = await connection.promise().query(SQL_STATEMENT, [answer.first_name, answer.last_name, selectedRole.id, selectedManager.id]);
...
async function addRole(departments) {
    try {
        ...
        console.log(answer);
        let thisId = departments.find((department) => department.name === answer.department)
        ...
        const [rows, fields] = await connection.promise().query(SQL_STATEMENT, [answer.title, answer.salary, thisId]);
        return rows;
    } catch (error) {
        console.log(error);
    }
